...
import { matchesApi, type ApiMatch } from "@/lib/api/client";
...
function matchInFilter(m: ApiMatch, filter: StageFilter): boolean {
  if (filter === "All") return true;
  if (filter === "Group Stage") return m.stage.startsWith("Group");
  return m.stage.toLowerCase().includes(filter.toLowerCase());
}

// Convert ApiMatch to the shape MatchCard expects
...
function SchedulePage() {
  ...
  useEffect(() => {
    matchesApi
      .getAll()
      .then((data) => setMatches(data.map(toViewModel)))
      .catch(() => setError("Can't reach the server. Check your connection — the tournament hasn't been cancelled yet."))
      .finally(() => setLoading(false));
  }, []);

  const filtered = useMemo(() => {
    return matches.filter((m) => {
      if (!matchInFilter(m as unknown as ApiMatch, filter)) return false;
      if (search.trim()) {
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 space-y-10">
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="mt-4 text-chalk-dim">Loading matches...</p>
          </div>
        )}

        {error && (
          <div className="text-center text-chalk-dim py-20">{error}</div>
        )}
        ...
        {grouped.map(([day, dayMatches]) => (
          <section key={day}>
            <div className="flex items-baseline gap-3 mb-4">
              ...
              <span className="text-chalk-dim text-sm">[{dayMatches.length} matches]</span>
