    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      {top3.length > 0 && (
        <div className="mt-8 grid grid-cols-3 gap-4 items-end max-w-xl mx-auto">
          <PodiumSpot row={top3[1]} place={2} height="h-32" />
          <PodiumSpot row={top3[0]} place={1} height="h-44" leader />
          <PodiumSpot row={top3[2]} place={3} height="h-24" />
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="font-scoreboard text-2xl text-chalk-dim">
                {r.rank}
                {r.rank === 1 && <Crown className="inline ml-1 size-4 text-gold" />}
              </div>
...
function PodiumSpot({
  ...
  return (
    <div className="flex flex-col items-center text-center">
      <div className="text-4xl mb-1">{row.avatar}</div>
      ...
      <div className={`font-scoreboard text-2xl ${leader ? "text-gold" : "text-chalk"}`}>
        {row.total_pts} pts
      </div>
      <div
