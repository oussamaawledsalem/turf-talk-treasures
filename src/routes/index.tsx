...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "KICKOFF — World Cup 2026 Prediction Game" },
      {
...
function AuthPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    const result = tab === "login" ? login(username, password) : signup(username, password, avatar);
    if (!result.ok) {
        ...
        <div>
          ...
          <div className="mt-10 max-w-md rounded-2xl border border-turf-line bg-turf-mid/90 backdrop-blur p-6">
            ...
            <form onSubmit={submit} className="flex flex-col gap-4">
                      ...
                      <button
                        ...
                        className={`h-12 rounded-lg border text-2xl transition ${
                          avatar === a
                            ...
                            : "border-turf-line hover:border-chalk-dim"
                        }`}
                      >
                        {a}
                      </button>
              ...
              {error && (
                <div className="text-red-card text-sm">❌ {error}</div>
              )}
              ...
              <motion.button
                ...
                className="mt-2 w-full rounded-lg bg-gold py-3 font-display text-2xl tracking-wider text-turf-deep hover:bg-floodlight transition"
              >
                {tab === "login" ? "Enter the pitch →" : "Join the game →"}
              </motion.button>
