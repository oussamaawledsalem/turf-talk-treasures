...
import type { Match } from "@/lib/schedule-data";
import { usePredictions } from "@/lib/predictions-context";
import { calculatePoints, matchHasStarted, type Prediction } from "@/lib/scoring";
...
export function MatchCard({ match }: { match: Match }) {
  ...
  const lock = () => {
    setPrediction(match.id, scoreA, scoreB);
    setEditing(false);
    setFlash(true);
  ...
  else if (hasResult && points === 0)
    borderClass = "border-l-[3px] border-l-red-card border-turf-line";
        ...
        <div className="mt-5 rounded-xl border border-turf-line bg-turf-deep/70 p-4">
          {hasResult ? (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <button
                onClick={lock}
                className="inline-flex items-center gap-2 rounded-lg bg-gold px-5 py-2.5 font-display text-xl tracking-wider text-turf-deep hover:bg-floodlight transition"
              >
                <Lock className="size-4" /> Lock in prediction
              </button>
            </div>
          ) : (
